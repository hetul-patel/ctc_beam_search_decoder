...
#ifndef CUSTOM_CTC_CTC_BEAM_SEARCH_H
#define CUSTOM_CTC_CTC_BEAM_SEARCH_H
...
public:
    ...
    int TopPaths(int n, std::vector<std::vector<int>>* paths,
...
private:
    ...
    DISALLOW_COPY_AND_ASSIGN(CTCBeamSearchDecoder);
};

template <typename T, typename CTCBeamState, typename CTCBeamComparer>
int CTCBeamSearchDecoder<T, CTCBeamState, CTCBeamComparer>::Decode(
    ...
    if (std::any_of(output->begin(), output->end(),
        ...
        return -1; //"output needs to be of size at least (top_n, batch_size)."
    }
    if (scores->rows() < this->batch_size_ || scores->cols() < top_n) {
        return -1;// "scores needs to be of size at least (batch_size, top_n)."
    }
    ...
    for (int b = 0; b < this->batch_size_; ++b) {
        ...
        int status = TopPaths(top_n, &beams, &beam_log_probabilities, this->merge_repeated_);
...
void CTCBeamSearchDecoder<T, CTCBeamState, CTCBeamComparer>::Step(
    ...
    if (top_k) {
        ...
        max_coeff = raw_input.maxCoeff(); // hetul : maximum score in given row
    }
    ...
    for (int j = 0; j < raw_input.size(); ++j) {
        ...
        //logsumexp += exp(raw_input(j) - max_coeff);
    }
    ...
    const T label_selection_input_min = //hetul:-inf
...
template <typename T, typename CTCBeamState, typename CTCBeamComparer>
int CTCBeamSearchDecoder<T, CTCBeamState, CTCBeamComparer>::TopPaths(
        int n, std::vector<std::vector<int>>* paths, std::vector<T>* log_probs,
        bool merge_repeated) const {
    paths->clear();
    log_probs->clear();
    if (n > beam_width_) {
        return -1; //errors::InvalidArgument("requested more paths than the beam width.");
    }
    if (n > leaves_.size()) {
        return -1; //"Less leaves in the beam search than requested.");
    }
...
#endif //CUSTOM_CTC_CTC_BEAM_SEARCH_H
